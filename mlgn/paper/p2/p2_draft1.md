...
† control at keep-bias 3; no kb0-matched 3-seed control set exists — listed for orientation, not as a matched arm. Gated s0 is the 2026-06-18 run (older logging schema); s1/s2 are July runs. `[TODO optional: +2 seeds/arm + seeded equal-gates control — queued, run_queue_p2.sh tier 2.1/2.2.]`
...
- **arXiv v1:** this document minus internal blocks, after: [ ] FIG 1–3 rendered, [ ] `run_queue_p2.sh` tier 1 completed and tables refreshed (tier 2 optional), [ ] exporter decision resolved, [ ] merge p2→main + tag commit `p2-draft1-numbers`.
- **Remaining-runs queue:** `mlgn/paper/p2/run_queue_p2.sh` (tier 1 = claim repair: distcopy s2 ×4, kb-matched selcopy pair ×2; tier 2 = optional: psMNIST seeds/controls ×8, parity-L16 panel ×10, psMNIST+ds probe ×3). Blocked-on-code items (float-GRU baseline, n-bit memorization, netlist exporter) are listed in its header, not queued.
