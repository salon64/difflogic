...
**Scope.** This is evidence on hold tasks with valid per-step targets, at a task (copy-50) that *saturates* once trainable. Deep supervision is not a universal gap eraser: on distcopy one gated seed keeps a +0.122 gap despite it (§7.2), and our headline psMNIST-28 runs do not use it — for cause: adding it there *costs* 6.8 points (gated kb0, disc 0.616 → 0.549 over 3 seeds), because supervising every step against the final label is exactly wrong for a task whose answer accumulates over time. The claim is: *per-step supervision repairs long-horizon write/hold credit assignment, which is the actual barrier at 50 steps* — not "deep supervision closes all gaps."
...
### 7.2 Accuracy across tasks: no consistent winner (the honest map)
...
| distcopy L50 D=8 + ds (kb3 both, n=3) | **0.955** | 0.875 | gated ahead (clatch-s1 outlier 0.754) |
| distcopy L50 D=20 + ds (kb3 both, n=3) | 0.874 | **0.916** | clatch ahead (clatch-s2 outlier 1.000) |
| psMNIST-28 (kb0 both, n=5) | 0.616 | 0.643 | tie (Welch t≈0.97; excl. gated-s2: 0.641 vs 0.643) |
| parity-16/32 + running-target ds (kb0, n=3 each) | ≈0.50 (chance) | ≈0.50 (chance) | both fail; see mechanism panel |
| selcopy L50/L100 (confounded) | 0.626 / 0.499 | 0.494 / 0.375 | **excluded** (see below) |

**The primitive-separator hunt failed, and we say so.** Two purpose-built separators — parity with dense running-XOR supervision, and distractor-copy with matched keep-bias — were designed to let a clean register beat a leaky soft MUX. Neither separated, and with three seeds the outliers fall **symmetrically**: at D=8 the gated mean leads (0.955 vs 0.875) because one clatch seed under-trains (0.754); at D=20 the clatch mean leads (0.916 vs 0.874) because one clatch seed is perfect (1.000); every other seed pair ties to three decimals. On parity only the **T flip-flop** moves off chance (L=32: disc 0.572/0.582/0.586; L=16: 0.576/0.607/0.607 with soft up to 0.936 — the toggle primitive on its home task, still far from solving), while clatch, gated and the control stay at 0.50 at both lengths. The parity panel is a *mechanism* result — the dense target unlocks exactly the primitive whose inductive bias matches the task — not an accuracy win for any cell we advocate. **There is no task with a consistent accuracy separation in either direction.** `[FIG 5 — parity mechanism panel, L∈{16,32}.]`
...
**Distcopy gap footnote (closed).** The tentative sign asymmetry died with the third seed: the gated cell now also shows a negative gap on distcopy (d8-s2: −0.120), and clatch shows positive gaps elsewhere (+0.112 selcopy-L100, +0.041 psMNIST kb1). There is no gap-sign law in either direction; negative gaps on both arms trace to a worse soft optimum that rounding recovers at the same discrete accuracy.

### 7.3 Training stability: the matched pair corrects the census

Our destabilization signal is `n_skipped` — optimizer steps skipped because the global gradient norm went non-finite (§3.2). An observational census over the pre-backfill archive was suggestive: **every destabilized run was `gated`** (five of them: 16,695 and 13,339 skips/20k at lr 0.01, 1,477/30k at hidden 2048, 903/20k with margin-reg, and 2,082/20k at selcopy-L100), while the register family had 0 skips in 75 runs — including everywhere it trains to high accuracy. But four of those five explosions sat at configurations the register family never ran, and the one shared-stratum comparison (selcopy-L100) was keep-bias-mismatched (gated kb3 vs. clatch kb1), with keep-bias a documented explosion driver. So we ran the controlled pair the census implied: each mechanism at the *other's* keep-bias, everything else identical.

**The pair refuted the census.** At keep-bias 3, `clatch` destabilizes *worse* than the gated run had (**8,044** of 20,000 steps skipped; discrete accuracy 0.497) — the hard-identity hold does not protect it. At keep-bias 1, `gated` is perfectly stable (0 skips) and posts the best selcopy-L100 discrete accuracy of any run in the archive (0.609). The full census now reads: gated 5/93 runs destabilized, clatch 1/31, latch 0/28, combo 0/29, rddlgn 0/27 — and every destabilization, on either mechanism, sits at high keep-bias and/or aggressive optimization on a long horizon.

**Claim (corrected).** *Training instability in recurrent LGNs follows keep-bias at long horizon, not the memory mechanism.* Our own prior hypothesis — that the explosion pathway runs through the soft multiplicative hold, which `clatch` replaces with a hard identity — is refuted by the pair: the enable and candidate networks still receive keep-bias-shaped gradients over the full unroll regardless of how the hold is computed, and that is where the blow-up lives. What survives for practice: (i) the skip-step guard turns these failures graceful rather than fatal; (ii) every configuration used for a table in this paper trained with 0 skipped steps on every arm; (iii) keep-bias must be tuned *down* as the horizon grows — the same task-dependence P1 established for accuracy, now with a stability face. Stability is a **diagnosis** in this paper, not an edge for the register. The observational census that suggested otherwise is exactly why controlled pairs matter; we report the correction rather than the census.

### 7.4 Deploy-consistency: the discretization gap at matched configuration

**Table 3 — psMNIST-28, keep-bias 0 for all arms, 5 seeds, gate-matched (4,000).** disc (per seed) and gap (per seed).

| mechanism | disc s0..s4 (mean) | gap s0..s4 (mean) |
|---|---|---|
| `gated` | .632 / .654 / .519 / .656 / .621 (.616) | +.077 / +.038 / **+.146** / +.051 / +.085 (+.079) |
| **`clatch`** | .657 / .647 / .598 / .656 / .657 (**.643**) | **+.023 / +.009 / +.027 / +.026 / +.016 (+.020)** |
| `combo` | .606 / .659 / .607 / .666 / .627 (.633) | +.021 / −.014 / −.025 / −.007 / +.001 (−.005) |
| `rddlgn` (equal-gates, n=3) | .655 / .686 / .681 (**.674**) | +.038 / +.032 / +.030 (+.034) |

The control row is now a seeded, gate-matched arm (hidden 2000 = 4,000 gates; keep-bias is a no-op for a cell with no gate network) — and it is in fact the **most accurate** model on this integration task, consistent with P1's equal-gates finding that gating buys recall, not classification. The mechanism table is about the gap, not about winning psMNIST. Gated s0 is the 2026-06-18 run (older logging schema); all other seeds are July runs.

At statistically indistinguishable accuracy between the two gated-family arms (clatch .643 vs gated .616, Welch t≈0.97 — the difference is one gated outlier seed, and excluding it gives .641 vs .643), the clatch gap is **bounded and non-overlapping with the gated gap across all five seeds** (max clatch +0.027 < min gated +0.038; means 3.9× apart, 3.1× excluding the gated outlier). The whole restore family shows the same cleanliness (combo gaps within ±0.025), and the register's gap band sits below the control's (+0.030..+0.038) as well.
...
1. **No consistent accuracy result for the register — in either direction.** Per-task means flip on single-seed outliers (gated ahead at distcopy-d8, the register ahead at d20); nothing separates, and our two purpose-built separators failed to produce a separation. The contributions are the mechanism (§4), the method (§6.1), and the deploy-consistency property (§7.4); the stability edge we once hypothesized was refuted by our own controlled pair (§7.3).
...
3. **Seed counts.** After the backfill queue, distcopy is n=3 and psMNIST-28 is n=5 per arm; the psMNIST gated-s0 run predates the others (older schema). The kb-matched selcopy stability pair is n=1 per cell — directional, not definitive.
4. **Stability is a diagnosis, not a result in the register's favor.** The census is observational, and its controlled follow-up refuted the register-stability reading (§7.3). The surviving claim — instability tracks keep-bias at long horizon, mechanism-independently — rests on one matched pair plus the census pattern; a keep-bias × horizon sweep would pin it properly.
...
| Table 2 accuracy map | `distcopy_*_dc_*_d{8,20}_s{0,1,2}_*`, `psmnist_*_psm_*_kb0_s{0..4}_*`, `parity_*_pd_*` + `parity_*_pd16_*`, `selcopy_*_bh_selcopy_*` |
| Table 3 psMNIST gaps | `psmnist_{gated,clatch,combo}_psm_*_kb0_s{0..4}_*` + `psmnist_gated_psm28_gated_kb0_20260618-*` (s0) + `psmnist_rddlgn_psm28_rddlgn_eqgates{,_s1,_s2}_*` |
| stability census + matched pair | all 218 JSONs, `n_skipped` field (gated 5/93, clatch 1/31, latch 0/28, combo 0/29, rddlgn 0/27); the pair = `selcopy_gated_sc_gated_L100_kb1_*` (0 skips) vs `selcopy_clatch_sc_clatch_L100_kb3_*` (8,044 skips) |
| §6.1 ds-hurts-integration control | `psmnist_gated_psm_gated_kb0_ds_s{0,1,2}_*` (0.549 mean vs 0.616 without) |
...
- **arXiv v1:** this document minus internal blocks, after: [ ] FIG 1–3 rendered, [x] `run_queue_p2.sh` tiers 1–2 completed (2026-07-09) and all tables refreshed against the 218-JSON archive, [ ] exporter decision resolved, [ ] tag commit `p2-draft1-numbers`.
- **Backfill queue: DONE (2026-07-09, commit 9e52847).** All 27 runs landed; logs in `mlgn/paper/p2/logs/`. Outcome summary in `research/04_experiment_log.md` (2026-07-09 entry): the kb-matched pair **refuted the stability edge** (clatch@kb3 skipped 8,044 — the draft's §7.3 was rewritten from "register never destabilizes" to "instability follows keep-bias, not mechanism"); the psMNIST gap non-overlap **held at 5 seeds** (3.9×); distcopy outliers came out symmetric (gated ahead d8, clatch ahead d20). Still blocked-on-code: float-GRU baseline, n-bit memorization task, netlist exporter (see the script header).
- Known reviewer pressure points and their in-text answers: accuracy-tie (→ integrity statement §1, §7.2), stability (→ §7.3 reports the controlled refutation of our own census — the correction *is* the answer), by-construction gap (→ design-property framing §7.4), "clatch = gated at deploy" (→ owned in §6.2), no float baseline (→ Limitations 6 + TODO), chunked psMNIST naming (→ §3.3, Limitations 7).
