...
| [25_uav_detector_scout.md](25_uav_detector_scout.md) | **UAV-LGN detector scout (2026-07-18, 8-agent adversarial).** Drone thesis reframed control→detection after the flight-control gate failed. Verdict: **(a) UAV FAULT detection CONDITIONAL ~60%** (verification-of-a-learned-circuit is the ONLY surviving moat), **(b) GPS-spoofing NO-GO** (R2U2 + ~0.998-F1 saturation + Tsetlin race). Recurrence REFUTED on UAV telemetry (AeroTSBoost) → windowed-FF LGN. Must-cites now realized: **RDDLGN published (ETH race risk), R2U2 (verification competitor for all of P3a), CLGN (Princeton FPGA-anomaly flag).** |
| [23_p3b_workmap.md](23_p3b_workmap.md) | **P3b work map (2026-07-11): hardware realization + Kyushu thesis. DECIDED: (1) Kyushu pitch leads; tool paper on the shared trunk (head-in-fabric → P&R/Fmax/nJ → board demo), arXiv-first, FPL'27 target / FCCM'27 stretch. (2) Benchmark carrier = CAN-bus IDS (doc-22's flagship pulled INTO P3b; recurrence-earns-its-keep gate first); flight sim = Kyushu thesis-gate only.** Drone = research extension, automotive = commercial extension; bearings/HFT/NID/EEG = gated portfolio; pivot rules pre-committed. |
| [24_roadmap.md](24_roadmap.md) | **Portfolio roadmap (2026-07-12): the paper & product pool.** Everything docs 19/21/22 left standing as grab-ready outlines — papers `../paper/p4..p14/init.md` (verdict, honest venue tier, gate, must-reads; no timeline by design), products `../product/*/init.md` (can-ids, bearing-monitor, lgn-toolchain OSS-first, hft parked). Dependency map, cheap-gate queue (p5 afternoon → p6 day → p9 days), parked/dead table, race watch. |
| [25_outreach_plan.md](25_outreach_plan.md) | **Outreach & networking plan (2026-07-15, 26-agent verified).** Whom to connect with on LinkedIn/email and WHEN: no bare adds pre-P1 (leak mechanics + PIs-use-email norms); safe-list now (Vargas, imec, DWN/UT-Austin, EmLogic/Umuroglu, Transcendent Logic, Miotti); main wave at P1-arXiv; PI channel = email-with-artifact. Key finds: **Wattenhofer part-time at ETH (Anza)**, ISTA deadline Jan 8 '27/'28 + Lampert says don't pre-contact, **UMD = third racing group (R-DTLGN+Belta)**, **Emily Yu = new PI target (Leiden, verification)**, Petersen still postdoc (+2 LGN patents pending), NeurIPS'26 = Sydney + Paris satellite. |
...
**P3a (verification) executed 2026-07-10/11** — pipeline + theorems in `../netlist/`,
skeleton in `../paper/p3a/`. **P3b kicked off 2026-07-11** (Kyushu leads) — plan in
[23_p3b_workmap.md](23_p3b_workmap.md).
